...
bool KobukiController::init() {
  ...
  go_to_certain_cell_service = nh.advertiseService("go_to_certain_cell", &KobukiController::handleGoToCertainCell, this);
...
bool KobukiController::moveToCell(Cell cell, int direction) {
  float distance = cell.getCellSize() * 0.024;
...
bool KobukiController::handleGoToCertainCell(is_kobuki::GoToCertainCell::Request& request,
                    is_kobuki::GoToCertainCell::Response& response) {
  std::vector<Cell> cellArr;
  std::vector<int> ints = (std::vector<int>) request.cells;
  int row, col;
  for(int i = 0; i < ints.size(); i++) {
    row = ints.at(i) / 100;
    col = ints.at(i) - row * 100;
    cellArr.push_back(Common::cells[row][col]);
  }
